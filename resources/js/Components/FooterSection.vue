<template>
    <footer class="bg-white">
        <div class="mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-24 lg:px-8">
            ...
            <div class="mt-10 flex justify-center space-x-10">
                <a v-for="item in socialLinks" :key="item.name" :href="item.href" class="text-gray-400 hover:text-gray-500" :class="hoverClass(item.name)" target="_blank" rel="noopener noreferrer">
                    <span class="sr-only">{{ item.name }}</span>
                    <component :is="item.icon" class="h-6 w-6" aria-hidden="true" />
                </a>
            </div>
            <p class="mt-10 text-center text-xs leading-5 text-gray-500">&copy; {{ currentYear }} {{ siteSettings.site_name }}, Inc. All rights reserved.</p>
        </div>
    </footer>
</template>
