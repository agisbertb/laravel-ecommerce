        ...
        <div class="bg-white">
            <main class="pb-24">
                ...
                <Disclosure as="section" aria-labelledby="filter-heading"
                    class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 items-center">
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="py-1">
                                            <MenuItem v-for="option in sortOptions" :key="option.name"
                                                v-slot="{ active }">
                                            <a @click.prevent="sortBy(option.order)"
                    ...
                    <div class="border-b border-gray-200">
                        <div class="-mx-px grid grid-cols-2 sm:mx-0 md:grid-cols-3 lg:grid-cols-4">
                            <div v-for="product in products.data" :key="product.id" class="group relative p-4 sm:p-6">
                                <div
                                    class="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg bg-gray-200 duration-300 ease-in-out group-hover:scale-110">
                                    <Link :href="route('products.show', product.slug)">
                                    <img :src="product.image_url" :alt="product.imageAlt"
                                    ...
                                    <div class="mt-3 flex flex-col items-center">
                                        ...
                                        <div class="flex items-center">
                                            <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating"
                                                :class="[product.average_rating > rating ? 'text-yellow-400' : 'text-gray-300', 'h-5 w-5 flex-shrink-0']"
                                                aria-hidden="true" />
                                        </div>
                ...
                <nav aria-label="Pagination"
                    ...
                    <div class="min-w-0 flex-1">
                        <a @click.prevent="changePage(products.prev_page_url)"
                            ...
                            :class="{
                                'cursor-not-allowed text-gray-400': !products.prev_page_url,
                    ...
                    <div class="hidden space-x-2 sm:flex">
                        <template v-for="page in [...Array(products.last_page).keys()].map(i => i + 1)" :key="page">
                            <a v-if="page === '...' || page === products.current_page || Math.abs(page - products.current_page) < 3"
                                @click.prevent="changePage(products.links[page].url)"
                                class="inline-flex h-10 items-center rounded-md border bg-white px-4 hover:bg-gray-100 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-opacity-25 focus:ring-offset-1 focus:ring-offset-blue-600 cursor-pointer"
                                :class="[page === products.current_page ? 'border-blue-600 ring-1 ring-blue-600' : 'border-gray-300']">
                                {{ page }}
                            </a>
                            <span v-else-if="Math.abs(page - products.current_page) === 3"
                                class="inline-flex h-10 items-center px-1.5 text-gray-500">...</span>
                        </template>
                    ...
                    <div class="flex min-w-0 flex-1 justify-end">
                        <a @click.prevent="changePage(products.next_page_url)"
                            class="inline-flex h-10 items-center rounded-md border border-gray-300 bg-white px-4 hover:bg-gray-100 focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-opacity-25 focus:ring-offset-1 focus:ring-offset-blue-600"
                            :class="{
                                'cursor-not-allowed text-gray-400': !products.next_page_url,
