...
function fetchOrders(query, order = orderBy.value) {
    router.get('/admin/orders', { search: query, order: order }, {
        preserveState: true,
        replace: true,
        preserveScroll: true,
    });
...
function sortBy(order) {
    orderBy.value = order;
    fetchOrders(searchQuery.value, order);
    showDropdown.value = false;
}
                                    ...
                                    <ul class="text-gray-700">
                                        <li>
                                            <button class="block px-4 py-2 hover:bg-gray-100 w-full text-left text-sm"
                                                    @click="sortBy('total_asc')">Total: Low to High</button>
                                        </li>
                                        <li>
                                            <button class="block px-4 py-2 hover:bg-gray-100 w-full text-left text-sm"
                                                    @click="sortBy('total_desc')">Total: High to Low</button>
                                        </li>
                                        <li>
                                            <button class="block px-4 py-2 hover:bg-gray-100 w-full text-left text-sm"
                        ...
                        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                                <table class="min-w-full divide-y divide-gray-300">
                                    ...
                                    <tr>
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-center text-gray-900">Order ID</th>
                                        ...
                                        <th scope="col" class="px-3 py-3.5 text-sm font-semibold text-center text-gray-900">Total</th>
                                    </tr>
                                    </thead>
                                    ...
                                    <tr v-for="order in orders.data" :key="order.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                        ...
                                        <td class="border-t px-6 py-4 text-center">
                                            <div class="flex items-center justify-center gap-x-2">
                                                <div :class="[statusClasses[order.status], 'flex-none rounded-full p-1']">
                                ...
                                <div class="flex items-center border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
                                    ...
                                    <div class="flex justify-end flex-grow">
                                        <nav class="inline-flex rounded-md shadow-sm" aria-label="Pagination">
                                            <a v-if="orders.prev_page_url" :href="orders.prev_page_url" class="relative inline-flex items-center rounded-l-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                                            ...
                                            <template v-for="page in paginationRange(orders.current_page, orders.last_page)" :key="page">
                                                <a :href="`?page=${page}`" :class="{
                                                    'bg-blue-600 text-white border-blue-600': page === orders.current_page,
                                                    'border-t border-b border-gray-300 text-gray-700 hover:bg-gray-50': page !== orders.current_page,
                                                    'border-r': page !== orders.last_page
                                                }" class="relative inline-flex items-center px-4 py-2 text-sm font-medium">
                                                    {{ page }}
                                                </a>
                                            </template>

                                            <a v-if="orders.next_page_url" :href="orders.next_page_url" class="relative inline-flex items-center rounded-r-md border border-gray-300 bg-white px-2 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
